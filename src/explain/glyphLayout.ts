 ...
 * case at certain fractional x OR y offsets (divide-by-zero in specific
 * quadratic segments) that corrupts a glyph's path into "NaN" coordinates —
 * rounding both to whole units avoids it entirely.
 */
export function layoutGlyphs(font: Font, text: string, x0: number, y: number, fontSize: number): LineLayout {
  ...
  for (const ch of text) {
    ...
    const path = glyph.getPath(Math.round(x), roundedY, fontSize);
